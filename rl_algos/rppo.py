...
try:
    ...
    from .common import ActorCriticRNNBetaPolicy
    ...
    from rl_algos.common import ActorCriticRNNBetaPolicy
...
class RPPO():
    ...
    def __init__(self,
        ...
        self.num_envs_per_batch = self.mini_batch_size // self.rollout_steps
        ...
        if self.policy_type == "Gaussian":
            ...
            self.policy = ActorCriticRNNBetaPolicy(
    ...
    def learn(self, total_timesteps: int, log_dir:str = None, project_name: str = None, trial_name: str = None, use_wandb: bool = False, log_frequency: int = 10):
        try:
            ...
            rnn_state = self.policy.initialize_rnn_state(batch_size=self.num_envs, device=self.device)
            while global_step < total_timesteps:
                initial_rnn_state = self.policy.clone_rnn_state(rnn_state)
                train_loop += 1
                # ALGO LOGIC: rollout
                sampling_start = time.time()
                for step in range(self.rollout_steps):
                    with torch.no_grad():
                        actions, logprobs, _, values, rnn_state = self.policy(obs, rnn_state, dones)
                        #print(actions)
                        #input()
                    ...
                    with torch.no_grad():
                        real_next_values = self.policy.value(real_next_obs, rnn_state, dones).view(-1)
                        if len(idx) > 0:
                ...
                b_returns,      \
                b_values,       \
                b_dones         = self.memory.sample()

                flat_idx = torch.arange(self.batch_size).reshape(self.rollout_steps, self.num_envs)
                clipfracs = []
                for _ in range(self.num_update_epochs):
                    # shullfe data
                    env_idx = torch.randperm(self.num_envs)
                    for idx0 in range(0, self.num_envs, self.num_envs_per_batch):
                        idxN = min(idx0 + self.num_envs_per_batch, self.num_envs)
                        mb_envs = env_idx[idx0:idxN]
                        mb_idx = flat_idx[:, mb_envs].ravel()
                        num_updates += 1
                        ...
                        _, newlogprob, entropy, newvalues, _ = self.policy(
                                b_obs[mb_idx], 
                                (initial_rnn_state[0][:, mb_envs], initial_rnn_state[1][:, mb_envs]) 
                                if isinstance(initial_rnn_state, tuple)
                                else initial_rnn_state[:, mb_envs],
                                b_dones[mb_idx],
                                b_actions[mb_idx])
                        ...
                        if torch.isnan(pi_loss) or torch.isnan(vf_loss) or torch.isnan(ent_loss):
