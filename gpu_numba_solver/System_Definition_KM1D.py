...
def _PA(t, x, cp):
    return cp[9]  * math.sin(2*math.pi*x) * math.sin(2*math.pi*t) \
         + cp[10] * math.sin(2*math.pi*x*cp[21]/cp[22]) * math.sin(2*math.pi*t*cp[12]/cp[11] + cp[13])


@cuda.jit(nb.float64(nb.float64, nb.float64, nb.float64[:]),
          device=True, inline=True)
def _PAT(t, x, cp):
    return  cp[9]  * cp[11] * math.sin(2*math.pi*x) * math.cos(2*math.pi*t) \
          + cp[10] * cp[12] * math.sin(2*math.pi*x*cp[21]/cp[22]) * math.cos(2*math.pi*t*cp[12]/cp[11] + cp[13])


@cuda.jit(nb.float64(nb.float64, nb.float64, nb.float64[:]),
          device=True, inline=True)
def _GRADP(t, x, cp):
    return cp[9]  * cp[19] * math.cos(2*math.pi*x) * math.sin(2*math.pi*t) \
         + cp[10] * cp[20] * math.cos(2*math.pi*x*cp[21]/cp[22]) * math.sin(2*math.pi*t*cp[12]/cp[11] + cp[13] )


@cuda.jit(nb.float64(nb.float64, nb.float64, nb.float64[:]),
          device=True, inline=True)
def _UAC(t, x, cp):
    return  cp[9]  * cp[23] * math.cos(2*math.pi*x) * math.cos(2 * math.pi * t) \
          + cp[10] * cp[23] * math.cos(2*math.pi*x*cp[21]/cp[22] ) * math.cos(2*math.pi*t*cp[12]/cp[11] + cp[13] )
...
def per_thread_ode_function(tid, t, dx, x, acc, cp):
    ...
    N = (cp[0] + cp[1]*x[1]) * p \
    - cp[2] * (1 + cp[7]*x[1]) - cp[3]* rx1 - cp[4]*x[1]*rx1 \
    - 1.5 * (1.0 - cp[7]*x[1] * (1.0/3.0))*x[1]*x[1] \
    - (1 + cp[7]*x[1]) * cp[5] * _PA(t, x[2], cp) - cp[6] * _PAT(t, x[2], cp) * x[0] \
    + 0.25 * x[3]*x[3]*cp[15]  
    ...
    # Translation Motion
    Fb1 = - cp[17]*x[0]*x[0]*x[0] * _GRADP(t, x[2], cp)            # Primary Bjerknes Force
    Fd  = - cp[18]*x[0] * (x[3]*cp[24] - _UAC(t, x[2], cp) )       # Drag Force

    # Composition of the ode functions
    dx[0] = x[1]
    dx[1] = N*rD
    dx[2] = x[3]
    dx[3] = 3*(Fb1+Fd)*cp[16]*rx1*rx1*rx1 - 3.0*x[1]*rx1*x[3] 
...
def per_thread_initialization(tid, t, td, x, acc, cp):
    ...
    acc[2] = x[2]      # Initial Bubble Position
...
def per_thread_finalization(tid, t, td, x, acc, cp):
    ...
    acc[3] = x[2]                   # Last Bubble Position
