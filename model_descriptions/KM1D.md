...
Note: The present model is the extension of model KM0D with 1 dimensional translational motion in standing acoustic wave field. 
...
where $R$ is the bubble radius, $z$ is the position of the bubble, $\rho_L$ is the liquid density, $c_L$ is the sound speed in the liquid domain, $p_L$ is the liquid pressure at the bubble wall and $p_{\infty}(z, t)$ is the excitation pressure calculated at the center of the bubble ($z$).
Note that the feedback term ($u^2/4$) from the translational motion is added to the right hand side [4].
First, the equation is rearranged to omit the time derivatives $d/dt$ on the right-hand side (for detalis see the model description of KM0D). The simplified form is given as:
...
The added mass force (or virtual mass force) resulting from the inertia of the surrounding liquid, and it is significant when the density of the surrounding liquid is greater than the density of the accelerating particle (i.e., gas particle in water). The virtual mass force is written as [3]:

$F_{m} =-\dfrac{\rho_L}{2}\dfrac{d}{dt}\left[V(u-v)\right]\approx-\dfrac{\rho_L}{2}V\ddot{x}-\dfrac{3}{2}\dfrac{\rho_L}{R}V\dot{R}\dot{x}$,

where $V=(4/3)\pi R^3$ is the bubble volume, $u=\dot{x}$ is the translational velocity of the bubble. $v$ is the velocity of the surrouding liquid that is negligible in the present case.
...
By substititing $F_m$ into the equation of motion and rearranging one obtains
...
For the present studdy, the the drag force is calculated as
...
One can use the trigonometric identity (Sum-to-product identities)
...
In the above quantities $\omega_r$ is a reference angular frequency and $\lambda_r$ is the corresponding reference wavelength. Tipically $\omega_r=\omega_0$, but we keep it as a free parameter during the derivation of the dimensionless systems.
...
x_1 \\
x_2 \\ 
\dfrac{1}{\tilde{D}_{KM}}\left(\tilde{N}_{KM} + C_{11}x_3^2\right) \\
C_{12} \cdot F_{ex}(x_1, \tau) \dfrac{3}{x_0^3} - 3\dfrac{x_3 x_2}{x_0}
\end{array}\right]
$
